...
if (!bibliotheque) {
    ...
    // Save initial books to sessionStorage
    saveLibrary();
}
...
function displayBooks() {
    ...
    for (let i = 0; i < bibliotheque.length; i++) {
        ...
        card.innerHTML = ` 
            ...
            <button class="btn-supprimer" data-code="${book.code}">Supprimer</button>`;
        
        databooks.appendChild(card);
    }
    
    // Add event listeners to delete buttons
    let deleteButtons = document.querySelectorAll('.btn-supprimer');
    for (let i = 0; i < deleteButtons.length; i++) {
        deleteButtons[i].addEventListener('click', function() {
            let code = parseInt(this.getAttribute('data-code'));
            deleteBook(code);
        });
    }
